...
- `data/source-registry.csv` currently holds 1413 entries.
...
The United Kingdom, Germany, France and Portugal completed the first four quality-consolidation pilots on 29 April 2026. Use that working-note pattern for the next high-yield pass: source-currency note, ENRIO status, main code baseline, annual-report or repository directory seed, funder matrix, boundary targets, stable app-data updates, source-registry entries and syntax checks. Greece is the strongest next calibration choice because it already signals institutional activity-report lanes, confidential local-board routes, EOF/CTIS boundary-publication questions, and a need to test whether annual-report clauses produce durable public files.
